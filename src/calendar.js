...
function setupCalendar(preferences) {
    window.mainApi.getLanguageDataPromise().then(async (languageData) => {
        calendar = await CalendarFactory.getInstance(
            preferences,
            languageData,
            calendar
        );
...
document.getElementById("sunday-hours").addEventListener("change", function () {
    this._preferences.hoursPerDay.Sunday = this.value;
});

document.getElementById("monday-hours").addEventListener("change", function () {
    this._preferences.hoursPerDay.Monday = this.value;
});
