...
class FlexibleMonthCalendar extends BaseCalendar {
    ...
    _getTargetDayForAllTimeBalance() {
        const targetYear = this._getCalendarYear(),
            ...
            isCurrentMonth =
                targetYear === this._getTodayYear() &&
                targetMonth === this._getTodayMonth(),
     ...
     */
    _getPageHeader() {
        const switchView = `<input id="switch-view" type="image" src="assets/switch.svg" alt="${this._getTranslation(
            "$BaseCalendar.switch-view"
        )}" title="${this._getTranslation(
            "$BaseCalendar.switch-view"
        )}" height="24" width="24"></input>`;
        const todayBut = `<input id="current-month" type="image" src="assets/calendar.svg" alt="${this._getTranslation(
            "$FlexibleMonthCalendar.current-month"
        )}" title="${this._getTranslation(
            "$FlexibleMonthCalendar.current-month"
        )}" height="24" width="24"></input>`;
        const leftBut = `<input id="prev-month" type="image" src="assets/left-arrow.svg" alt="${this._getTranslation(
            "$FlexibleMonthCalendar.previous-month"
        )}" height="24" width="24"></input>`;
        const rightBut = `<input id="next-month" type="image" src="assets/right-arrow.svg" alt="${this._getTranslation(
            "$FlexibleMonthCalendar.next-month"
        )}" height="24" width="24"></input>`;
        ...
        return (
            ...
            '<th class="th but-switch-view">' +
            switchView +
            "</th>" +
            '<th class="th but-left">' +
            leftBut +
            "</th>" +
            '<th class="th th-month-name" colspan="18"><div class="div-th-month-name" id="month-year"></div></th>' +
            '<th class="th but-right">' +
            rightBut +
            "</th>" +
            '<th class="th but-today">' +
            todayBut +
            "</th>" +
            "</tr></table>"
        );
    ...
    _getTableHeaderCode() {
        return (
            ...
            `<div class="header-day">${this._getTranslation(
                "$FlexibleMonthCalendar.day"
            )}</div>` +
            `<div class="header-day-total">${this._getTranslation(
    ...
    _getSummaryRowCode() {
        const leaveByCode =
            '<input type="text" id="leave-by" size="5" disabled>';
        return (
            '<div class="summary" id="summary-unfinished-day">' +
            `<div class="leave-by-text" colspan="7">${this._getTranslation(
                "$FlexibleMonthCalendar.leave-by"
            )}</div>` +
    ...
    _getBalanceRowCode() {
        return (
            ...
            `<div class="month-total-text" title="${this._getTranslation(
                "$FlexibleMonthCalendar.last-day-balance"
            )}">${this._getTranslation("$FlexibleMonthCalendar.on")}</div>` +
            `<div class="month-total-time" title="${this._getTranslation(
                "$FlexibleMonthCalendar.last-day-balance"
            )}"><span id="month-day-input"></span></div>` +
            `<div class="month-total-text" title="${this._getTranslation(
                "$FlexibleMonthCalendar.working-days-title"
            )}">${this._getTranslation(
                "$FlexibleMonthCalendar.working-days"
            )}</div>` +
            `<div class="month-total-time" title="${this._getTranslation(
                "$FlexibleMonthCalendar.working-days-title"
            )}"><span id="month-working-days"></span></div>` +
            `<div class="month-total-text" title="${this._getTranslation(
                "$BaseCalendar.month-balance-title"
            )}">${this._getTranslation("$BaseCalendar.month-balance")}</div>` +
            `<div class="month-total-time" title="${this._getTranslation(
                "$BaseCalendar.month-balance-title"
            )}"><input type="text" id="month-balance"     size="8" disabled></div>` +
            `<div class="month-total-text" title="${this._getTranslation(
                "$BaseCalendar.overall-balance-title"
            )}">${this._getTranslation(
                "$BaseCalendar.overall-balance"
            )}</div>` +
            `<div class="month-total-time" title="${this._getTranslation(
                "$BaseCalendar.overall-balance-title"
            )}"><input type="text" id="overall-balance" size="8" placeholder="..." disabled></div>` +
            "</div>"
        );
    }
    ...
    static _getRowCode(dateKey, isInterval = false) {
        ...
        return (
            '<div class="row-time">' +
            '<div class="ti" colspan="4"><input type="time" data-date="' +
            dateKey +
            '"></div>' +
            "</div>"
        );
    ...
    _getInputsRowCode(year, month, day) {
        ...
        if (waivedInfo !== undefined) {
            const summaryStr =
                `<b>${this._getTranslation(
                    "$FlexibleMonthCalendar.waived-day"
                )}: </b>` + waivedInfo["reason"];
            const waivedLineHtmlCode =
                '<div class="row-waiver" id="' +
                dateKey +
                '">' +
                '<div class="weekday">' +
                getDayAbbr(this._languageData.data, weekDay) +
                ...
                " </span>" +
                "</div>" +
                '<div class="waived-day-text" colspan="5">' +
                summaryStr +
                "</div>" +
        ...
        let htmlCode =
            "<div>" +
            `<div class="weekday waiver-trigger" title="${this._getTranslation(
                "$FlexibleMonthCalendar.add-waiver-day"
            )}">` +
            getDayAbbr(this._languageData.data, weekDay) +
            "</div>" +
            '<div class="day">' +
    ...
    _generateTableBody() {
        ...
        const monthLength = getMonthLength(
            this._getCalendarYear(),
            this._getCalendarMonth()
        );
        ...
        for (let day = 1; day <= monthLength; ++day) {
            if (
                ...
                this._getHideNonWorkingDays()
            ) {
                continue;
            }

            html += this._getInputsRowCode(
                this._getCalendarYear(),
                this._getCalendarMonth(),
                day
            );
    ...
    _drawArrowsAndButtons() {
        ...
        function toggleArrowColor(target) {
            ...
            element
                .parent()
                .find(".arrow")
                .toggleClass("disabled", !hasHorizontalScrollbar);
        }
        ...
        function removeEntries(element) {
            ...
            if (row.length > 3) {
                ...
                const removeEntriesDialogOptions = {
                    ...
                    message: `${this._getTranslation(
                        "$FlexibleMonthCalendar.entry-removal-confirmation"
                    )} ${dateKey}?`,
                    type: "info",
                    buttons: [
    ...
    refreshOnDayChange(oldDayDate, oldMonthDate, oldYearDate) {
        if (
            this._getCalendarMonth() === oldMonthDate &&
            this._getCalendarYear() === oldYearDate
        ) {
            this._goToCurrentDate();
        }
    }
    ...
    _updateBalance() {
        ...
        const monthTotalToWork = multiplyTime(
            this._getHoursPerDay(),
            workingDaysToCompute * -1
        );
        ...
        if (balanceElement) {
            ...
            balanceElement.addClass(
                isNegative(balance) ? "text-danger" : "text-success"
            );
        }
    ...
    _updateBasedOnDB() {
        ...
        const lastDateToCount = new Date(
            this._getCalendarYear(),
            this._getCalendarMonth(),
            this._getCalendarDate()
        );
        ...
        for (let day = 1; day <= monthLength; ++day) {
            ...
            const waivedInfo = this._getWaiverStore(
                this._getCalendarYear(),
                this._getCalendarMonth(),
                day
            );
            ...
            stopCountingMonthStats |=
                lastDateToCount.getDate() === day &&
                lastDateToCount.getMonth() === this._getCalendarMonth() &&
                lastDateToCount.getFullYear() === this._getCalendarYear();
    ...
    _updateLeaveBy() {
        ...
        if (dayTotal !== undefined && dayTotal.length > 0) {
            ...
            $("#leave-day-balance").addClass(
                isNegative(dayBalance) ? "text-danger" : "text-success"
            );
                ...
                if (validateTime(timeEnd) && validateTime(timeStart)) {
                    if (timeEnd > timeStart) {
                        $(element)
                            ...
                            .prev()
                            .find("span")
                            .html(subtractTime(timeStart, timeEnd));
                    }
                    timeStart = "";
                    timeEnd = "";
                }
    ...
    _setTableData(key) {
        ...
        for (const element of values) {
            if (indexIsInterval(i + 1)) {
                $(
                    this.constructor._getRowCode(key, true /*isInterval*/)
                ).appendTo("#" + key);
                i++;
            }
            ...
            let input = inputs[i];
            if (input === undefined) {
                input = $(this.constructor._getRowCode(key))
                    .appendTo("#" + key)
                    .find('input[type="time"]');
                input.val("");
            }
        ...
        while (
            !onlyOnePair &&
            (lessThanThreeEntries(i) || inputGroupFullyPrinted(i))
        ) {
            ++i;
            if (indexIsInterval(i)) {
                ...
                $(this.constructor._getRowCode(key))
                    .appendTo("#" + key)
                    .find('input[type="time"]');
            }
        }
    ...
    _hasInputError(key) {
        ...
        const validatedTimes = this._validateTimes(
            newValues,
        ...
        for (let index = 0; index < validatedTimes.length; index++) {
            if (
                index > 0 &&
                validatedTimes[index - 1] >= validatedTimes[index]
            ) {
                return true;
            }
        }
